...
# --------------------------------------------------------------
# СТАТИСТИКА (СОТРУДНИК, ТЕХНИК, АДМИНИСТРАТОР)
def statistics_view(request):
    ...
    #Статистика
    try:
        #ТОП из 10 оборудований по количеству поломок
        ...
        ).annotate(breakdown_count=Count('act_number')).order_by('-breakdown_count')[:10]
        #Статистика за пол года
        six_months_ago = timezone.now() - timedelta(days=180)
        monthly_stats = RequestFix.objects.filter(delete_date__isnull=True, registration_date__gte=six_months_ago
        ).extra(select={'month': "TO_CHAR(registration_date, 'YYYY-MM')"}
        ).values('month').annotate(count=Count('act_number')).order_by('month')
        #Общие расчеты: сколько в целом и сколько уже выполнено
        ...
        type_stats = RequestFix.objects.filter(
            delete_date__isnull=True
        ).values('equipment__type__name').annotate(count=Count('act_number')).order_by('-count')
        #Подготовка данных в HTML шаблон
        context = {
...
def manager_dashboard(request):
    ...
    if request.session.get('user_role') not in ['Руководитель', 'Администратор']:
        messages.error(request, 'Нет доступа')
        return redirect('login_view')
    try: #Общая статистика по количеству, по выполненным, по оставшимся
        total_requests = RequestFix.objects.filter(delete_date__isnull=True).count()
        ...
        ).extra(select={'month': "TO_CHAR(registration_date, 'YYYY-MM')"}).values('month').annotate(count=Count('act_number')
        ).order_by('month')
